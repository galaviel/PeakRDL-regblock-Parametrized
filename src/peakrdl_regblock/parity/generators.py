...
# galaviel TODO 'parity' is copied from 'readback' , perhaps make 'parity' inherit from it, and only over-ride the relevant functions?

class ParityLoopBody(LoopBody):
    """
...
class ParityAssignmentGenerator(RDLForLoopGenerator):
    i_type = "genvar"
    loop_body_cls = ParityLoopBody
    ...
    def process_reg(self, node: RegNode) -> None:
        ...
        for field in node.fields():
            if not field.get_property("paritycheck"):
                continue
            if not field.implements_parity:
                continue

            field_parity_error = self.exp.field_logic.get_field_parity_combo_identifier(field)
            
            self.add_content(f"parity_error |= {field_parity_error} ;")
